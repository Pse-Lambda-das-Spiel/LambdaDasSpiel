...
\item[Konstruktoren] \hfill \\
	\vspace{-.8cm}
	\begin{itemize}
		\item $\texttt{public \textbf{LambdaTermViewController}(LambdaRoot root, boolean editable)}$ \\ Instanziiert ein Objekt dieser Klasse mit dem gegebenen Lambda-Term. Fügt sich selber dem gegebenen Lambda-Term als Beobachter hinzu. Erstellt die Wurzel des View-Baumes vom Typ $\texttt{LambdaNodeViewController}$ und fügt dann rekursiv alle Knoten des gegebenen Lambda-Terms per $\texttt{ViewInsertionVisitor}$ dieser Wurzel hinzu. Erstellt den Event-Listener als anonyme innere Klasse, um Benutzerinteraktionen \(Drag-Event, Drop-Event\) zu verarbeiten.
		\begin{description}
			\item[Parameter] \hfill \\
			\vspace{-.8cm}
			\begin{itemize}
...
\item[Methoden] \hfill \\
	...
	\begin{itemize}
		...
		\item $\texttt{public void \textbf{setSelection}(LambdaTerm term)}$ \\ Erstellt einen neuen, nicht editierbaren $\texttt{LambdaNodeViewController}$ und speichert diesen in $\texttt{selection}$.
		\begin{description}
			\item[Parameter] \hfill \\
			\vspace{-.8cm}
			\begin{itemize}
		...
		\item $\texttt{public LambdaNodeViewController \textbf{getSelection}()}$ \\ Gibt den aktuell ausgewählten View-Knoten des Benutzers zurück oder $\texttt{null}$, falls kein Knoten ausgewählt ist.
		\begin{description}
			\item[Rückgabe] \hfill \\
			\vspace{-.8cm}
			\begin{itemize}
		...
		\begin{description}
			...
			\begin{itemize}
				...
				\item $\texttt{float x}$ \\ Die Y-Koordinate der Einfügeposition.
			\end{itemize}
			\item[Rückgabe] \hfill \\
			\vspace{-.8cm}
			\begin{itemize}
...
\item[Attribute] \hfill \\
	\vspace{-.8cm}
	\begin{itemize}
		\item $\texttt{private LambdaTerm \textbf{linkedTerm}}$ \\ Der Lambda-Term, der durch diesen View-Knoten angezeigt wird.
