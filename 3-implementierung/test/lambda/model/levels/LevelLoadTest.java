...
public class LevelLoadTest {
    ...
    public static void setUpBeforeClass() throws Exception {
        ...
        // Initialize the test start constellation: (lx.x)y, lx is blue, x is
        // white, y is white
        LambdaApplication startApplication = new LambdaApplication(start, true,
                false);
        LambdaAbstraction startAbstraction = new LambdaAbstraction(
                startApplication, Color.valueOf("0000ffff"), false);
        startAbstraction.setInside(new LambdaVariable(startAbstraction, Color
                .valueOf("0000ffff"), false));
        startApplication.setLeft(startAbstraction);
        startApplication.setRight(new LambdaVariable(startApplication, Color
                .valueOf("ffffffff"), false));
        ...
        // Initialize the hint start constellation: (lx.x)y, lx is blue, x is
        // blue, y is white
        LambdaApplication hintApplication = new LambdaApplication(hint, true,
                false);
        LambdaAbstraction hintAbstraction = new LambdaAbstraction(null,
                Color.valueOf("0000ffff"), false);
        hintAbstraction.setInside(new LambdaVariable(hintAbstraction, Color
                .valueOf("0000ffff"), false));
        hintApplication.setLeft(hintAbstraction);
        hintApplication.setRight(new LambdaVariable(hintApplication, Color
                .valueOf("ffffffff"), false));
        hint.setChild(hintApplication);
        testLevel = new LevelModel(2, start, goal, hint, tutorial,
                availableRedStrats, useableElements, 1, 10, 50, true, true,
                availableColors, lockedColors, ReductionStrategy.NORMAL_ORDER);
    ...
    public void testLoadLevel() {
        LevelModel jsonLevel = LevelLoadHelper.loadLevel(Gdx.files
                .internal("data/levels/02.json"));
        assertEquals(testLevel.getId(), jsonLevel.getId());
        assertEquals(testLevel.getCoins(), jsonLevel.getCoins());
        assertEquals(testLevel.getMaxReductionSteps(),
                jsonLevel.getMaxReductionSteps());
        ...
        for (int i = 0; i < jsonLevel.getTutorial().size(); i++) {
            assertEquals(testLevel.getTutorial().get(i).getId(), jsonLevel
                    .getTutorial().get(i).getId());
            assertEquals(testLevel.getTutorial().get(i).getImageName(),
                    jsonLevel.getTutorial().get(i).getImageName());
        }
        assertEquals(testLevel.getAvailableColors(),
                jsonLevel.getAvailableColors());
        assertEquals(testLevel.getLockedColors(), jsonLevel.getLockedColors());
        assertEquals(testLevel.getAvailableRedStrats(),
                jsonLevel.getAvailableRedStrats());
        assertEquals(testLevel.getUseableElements(),
                jsonLevel.getUseableElements());
        assertEquals(testLevel.getDefaultStrategy(),
                jsonLevel.getDefaultStrategy());
    }
    ...
    /**
     * Tests the loading of a sample level json file while using the
     * {@link LevelModelLoader}.
     */
    @Test
    public void testLevelModelLoader() {
        assets.setLoader(LevelModel.class, new LevelModelLoader(
                new InternalFileHandleResolver()));
        assets.load("data/levels/02.json", LevelModel.class);
        while (!(assets.update())) {
        }
        LevelModel loadedLevel = assets.get("data/levels/02.json",
                LevelModel.class);
        assertEquals(testLevel.getId(), loadedLevel.getId());
        assertEquals(testLevel.getCoins(), loadedLevel.getCoins());
        assertEquals(testLevel.getMaxReductionSteps(),
                loadedLevel.getMaxReductionSteps());
        ...
        for (int i = 0; i < loadedLevel.getTutorial().size(); i++) {
            assertEquals(testLevel.getTutorial().get(i).getId(), loadedLevel
                    .getTutorial().get(i).getId());
            assertEquals(testLevel.getTutorial().get(i).getImageName(),
                    loadedLevel.getTutorial().get(i).getImageName());
        }
        assertEquals(testLevel.getAvailableColors(),
                loadedLevel.getAvailableColors());
        assertEquals(testLevel.getLockedColors(), loadedLevel.getLockedColors());
        assertEquals(testLevel.getAvailableRedStrats(),
                loadedLevel.getAvailableRedStrats());
        assertEquals(testLevel.getUseableElements(),
                loadedLevel.getUseableElements());
        assertEquals(testLevel.getDefaultStrategy(),
                loadedLevel.getDefaultStrategy());
    }
