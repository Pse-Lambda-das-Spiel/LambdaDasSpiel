...
/**
 * This class helps with loading and initialising a ProfileModel. It loads the
 * general information, the settings, the statistics and on explicit invocation
 * the shop state of a profile from the specific profile directory.
 * 
...
public final class ProfileLoadHelper {
    ...
    /**
     * Loads and initialize the ProfileModel and its specific parts with the
     * given profile name. If no profile directory with the given name exists,
     * it returns null.
     * 
     * @param name
     *            the name of the to be loaded ProfileModel
     * @return the loaded and initialized ProfileModel
     * @throws IOException
     *             if there is an error while reading the profile json files
     */
    public static ProfileModel loadProfile(String name) {
        if (!(Gdx.files.local(ProfileManager.PROFILE_FOLDER + "/" + name)
                .exists())) {
            return null;
        }
        ProfileModel profile = new ProfileModel(name);
        FileHandle file = Gdx.files.local(ProfileManager.PROFILE_FOLDER + "/"
                + name + "/general_information.json");
    ...
    private static void loadIntoSettings(JsonValue settingsJson,
    ...
    private static void loadIntoStatistic(JsonValue statisticJson,
        ...
        statistic.setLambsEnchantedPerLevel(statisticJson
                .getInt("lambsEnchantedPerLevel"));
        statistic.setGemsEnchantedPerLevel(statisticJson
                .getInt("gemsEnchantedPerLevel"));
        statistic.setLambsPlacedPerLevel(statisticJson
                .getInt("lambsPlacedPerLevel"));
        statistic.setGemsPlacedPerLevel(statisticJson
                .getInt("gemsPlacedPerLevel"));
        statistic.setHintsNotUsed(statisticJson.getInt("hintsNotUsed"));
        statistic.setLevelTries(statisticJson.getInt("levelTries"));
        statistic.setSuccessfulLevelTries(statisticJson
                .getInt("successfulLevelTries"));
        statistic.setTimePlayed(statisticJson.getLong("timePlayed"));
    }

    /**
     * Loads the shop state of the profile associated with the given name into
     * the shop singleton.
     * 
     * @param name
     *            the name of the profile whose shop state should be loaded
     *            throws IOException if there is an error while reading the shop
     *            state json files
     */
    public static void loadIntoShop(String name) {
        ...
        for (JsonValue entry = shopJson.get("Musics").child(); entry != null; entry = entry.next) {
            MusicItemModel music = shop.getMusic().getItems()
                    .get(entry.getInt("MusicId"));
        ...
        for (JsonValue entry = shopJson.get("BKImages").child(); entry != null; entry = entry.next) {
            BackgroundImageItemModel bkImage = shop.getImages().getItems()
                    .get(entry.getInt("BKImageId"));
            bkImage.setPurchased(entry.getBoolean("purchased"));
            if (bkImage.getId()
                    .equals(shopJson.getString("activatedBKImageId"))) {
        ...
        for (JsonValue entry = shopJson.get("ElementUIContextFamilies").child(); entry != null; entry = entry.next) {
            ElementUIContextFamily elementUIContextFamily = shop
            ...
            if (elementUIContextFamily.getId().equals(
                    shopJson.getString("activatedElementUIContextFamilyId"))) {
                shop.getElementUIContextFamilies().setActivatedItem(
