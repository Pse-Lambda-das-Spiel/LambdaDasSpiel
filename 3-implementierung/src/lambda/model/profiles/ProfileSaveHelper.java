...
/**
 * This class helps with the saving of profiles into json files. It saves the
 * general information, settings, statistics and the shop state of a profile in
...
public final class ProfileSaveHelper {
     ...
     * 
     * @param profile
     *            the to be saved ProfileModel
     */
    public static void saveProfile(ProfileModel profile) {
        FileHandle a = Gdx.files.local(ProfileManager.PROFILE_FOLDER + "/"
                + profile.getName());
        if (!a.exists()) {
            a.mkdirs();
        }
    ...
    private static void saveGeneralInfos(ProfileModel profile) {
        ...
        profileJson.setSerializer(ProfileModel.class,
                new Json.Serializer<ProfileModel>() {
                    @Override
                    public void write(Json json, ProfileModel profileModel,
                            Class knownType) {
    ...
    private static void saveSettings(String profileName, SettingsModel settings) {
        ...
        settingsJson.setSerializer(SettingsModel.class,
                new Json.Serializer<SettingsModel>() {
                    @Override
                    public void write(Json json, SettingsModel settingsModel,
                            Class knownType) {
                        // create json structure
                        json.writeObjectStart();
                        json.writeObjectStart("profile_settings");
                        json.writeValue("musicOn", settingsModel.isMusicOn());
                        json.writeValue("musicVolume",
                                settingsModel.getMusicVolume());
                        json.writeValue("soundVolume",
                                settingsModel.getSoundVolume());
                        json.writeObjectEnd();
                        json.writeObjectEnd();
                    }
    ...
    private static void saveStatistic(String profileName,
            StatisticModel statistic) {
        Json statisticJson = new Json();
        statisticJson.setSerializer(StatisticModel.class,
                new Json.Serializer<StatisticModel>() {
                    @Override
                    public void write(Json json, StatisticModel statisticModel,
                            Class knownType) {
                        // create json structure
                        json.writeObjectStart();
                        json.writeObjectStart("profile_statistic");
                        json.writeValue("lambsEnchanted",
                                statisticModel.getLambsEnchanted());
                        json.writeValue("gemsEnchanted",
                                statisticModel.getGemsEnchanted());
                        json.writeValue("lambsPlaced",
                                statisticModel.getLambsPlaced());
                        json.writeValue("gemsPlaced",
                                statisticModel.getGemsPlaced());
                        json.writeValue("lambsEnchantedPerLevel",
                                statisticModel.getLambsEnchantedPerLevel());
                        json.writeValue("gemsEnchantedPerLevel",
                                statisticModel.getGemsEnchantedPerLevel());
                        json.writeValue("lambsPlacedPerLevel",
                                statisticModel.getLambsPlacedPerLevel());
                        json.writeValue("gemsPlacedPerLevel",
                                statisticModel.getGemsPlacedPerLevel());
                        json.writeValue("hintsNotUsed",
                                statisticModel.getHintsNotUsed());
                        json.writeValue("levelTries",
                                statisticModel.getLevelTries());
                        json.writeValue("successfulLevelTries",
                                statisticModel.getSuccessfulLevelTries());
                        json.writeValue("timePlayed",
                                statisticModel.getTimePlayed());
                        json.writeObjectEnd();
                        json.writeObjectEnd();
                    }

                    @Override
                    public StatisticModel read(Json json, JsonValue jsonData,
                            Class type) {
                        return null;
                    }
                });
        statisticJson.setOutputType(OutputType.json);
        FileHandle statisticFile = Gdx.files
                .local(ProfileManager.PROFILE_FOLDER + "/" + profileName
                        + "/statistic.json");
        ...
        shopJson.setSerializer(ShopModel.class,
                new Json.Serializer<ShopModel>() {
                    ...
                    public void write(Json json, ShopModel shopModel,
                        ...
                        if (shopModel.getElementUIContextFamilies()
                                .getActivatedItem() != null) {
                            json.writeValue(
                                    "activatedElementUIContextFamilyId",
                                    shopModel.getElementUIContextFamilies()
                        ...
                        if (shopModel.getMusic().getActivatedItem() != null) {
                            json.writeValue("activatedMusicId", shopModel
                                    .getMusic().getActivatedItem().getId());
                        } else {
                            json.writeValue("activatedMusicId", "");
                        }
                        if (shopModel.getImages().getActivatedItem() != null) {
                            json.writeValue("activatedBKImageId", shopModel
                                    .getImages().getActivatedItem().getId());
                        } else {
                            json.writeValue("activatedBKImageId", "");
                        }
                        json.writeArrayStart("musics");
                        for (MusicItemModel music : shopModel.getMusic()
                                .getItems()) {
                            json.writeObjectStart();
                        ...
                        for (BackgroundImageItemModel image : shopModel
                                .getImages().getItems()) {
                            json.writeObjectStart();
                        ...
                        for (ElementUIContextFamily elementUIContextFamily : shopModel
                                .getElementUIContextFamilies().getItems()) {
                            json.writeObjectStart();
                            json.writeValue("ElementUIContextFamilyId",
                                    elementUIContextFamily.getId());
                            json.writeValue("purchased",
                                    elementUIContextFamily.isPurchased());
                            json.writeObjectEnd();
                        }
                        json.writeArrayEnd();
                        json.writeObjectEnd();
                    ...
                    @Override
                    public ShopModel read(Json json, JsonValue jsonData,
                            Class type) {
                        return null;
                    }
                });
