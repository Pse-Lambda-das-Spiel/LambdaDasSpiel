...
public class SiblingInserter implements LambdaTermVisitor {
	...
	public SiblingInserter(LambdaTerm sibling, boolean left) {
		...
		if (sibling instanceof LambdaVariable) {
			int temp = ProfileManager.getManager().getCurrentProfile()
					.getStatistics().getGemsEnchanted();
			ProfileManager.getManager().getCurrentProfile().getStatistics()
					.setGemsEnchanted(temp + 1);
		}
