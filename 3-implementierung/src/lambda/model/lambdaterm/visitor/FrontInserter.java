...
public class FrontInserter implements LambdaTermVisitor {
	...
	public FrontInserter(LambdaTerm inserted) {
		...
		//update Statistics 
	        	if(inserted instanceof LambdaAbstraction  ){
	        		int temp = ProfileManager.getManager().getCurrentProfile().getStatistics().getLambsEnchanted();
	        	     ProfileManager.getManager().getCurrentProfile().getStatistics().setLambsEnchanted(temp+1);
	        	}
	}
