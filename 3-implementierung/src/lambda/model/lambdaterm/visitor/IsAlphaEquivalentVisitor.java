...
/**
 * Represents a visitor on a lambda term that checks if it is equivalent with
 * another lambda term. Can only check valid lambda terms.
 * 
...
public class IsAlphaEquivalentVisitor extends ValidLambdaTermVisitor<Boolean> {
    ...
    /**
     * Stores the color translations of bound colors from the visited term to
     * the other term. All elements are arrays of length 2. Everytime an
     * abstraction is visited the color translation is pushed on the stack and
     * popped when the abstraction is left.
     */
    ...
    public IsAlphaEquivalentVisitor(LambdaTerm other) {
        super(
                "Cannot perform an alpha equivalence check on an invalid lambda term!");
        if (other == null) {
            throw new IllegalArgumentException("Lambda term cannot be null!");
        }
        if (!other.accept(new IsValidVisitor())) {
            throw new InvalidLambdaTermException(
                    "Cannot perform an alpha equivalence check on an invalid lambda term!");
        }
        this.other = other;
        result = true;
    ...
    /**
     * Visits the given lambda root and compares with other for alpha
     * equivalence.
     * 
     * @param node
    ...
    /**
     * Visits the given lambda application and compares with other for alpha
     * equivalence.
     * 
     * @param node
    ...
    /**
     * Visits the given lambda abstraction and compares with other for alpha
     * equivalence.
     * 
     * @param node
    ...
    public void visitValid(LambdaAbstraction node) {
        ...
        if (result) {
            // Push to stack: Translation from visited color to other term's
            // color
            colorMap.push(new Color[] {
                    node.getColor(), ((LambdaAbstraction) other).getColor() });
    ...
    /**
     * Visits the given lambda variable and compares with other for alpha
     * equivalence.
     * 
     * @param node
    ...
    public void visitValid(LambdaVariable node) {
        ...
        if (result) {
            ...
            boolean bound = false; // Indicates whether the visited variable is
                                   // bound
            ...
            for (Color[] colorTranslation : colorMap) {
                ...
                if (colorTranslation[0].equals(node.getColor())) {
                    bound = true;
                    // Visited variable is bound => color of other variable
                    // should be translated color
     ...
     * 
     * @return true if the visited and other terms are alpha equivalent, false
     *         otherwise
     */
    @Override
