...
public final class EditorViewController extends StageViewController implements
	...
	public void reset(LevelContext context) {
		...
		abstraction.setChild(new LambdaAbstraction(abstraction, Color.WHITE,
				true));
		toolbarElements[0] = LambdaTermViewController.build(abstraction, false,
				model.getLevelContext(), getStage(), false);
		LambdaRoot application = new LambdaRoot();
		application.setChild(new LambdaApplication(application, true));
		toolbarElements[1] = LambdaTermViewController.build(application, false,
				model.getLevelContext(), getStage(), true);
		LambdaRoot variable = new LambdaRoot();
		variable.setChild(new LambdaVariable(variable, Color.WHITE, true));
		toolbarElements[2] = LambdaTermViewController.build(variable, false,
				model.getLevelContext(), getStage(), false);
		for (LambdaTermViewController toolbarElement : toolbarElements) {
			term.addPermanentDragSource(new LambdaTermDragSource(toolbarElement
					.getRoot().getChild(0), false, term));
		}
		bottomToolBar.clear();
