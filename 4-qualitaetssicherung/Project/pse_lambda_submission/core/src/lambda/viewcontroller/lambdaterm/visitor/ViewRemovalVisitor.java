...
/**
 * Represents a visitor on a lambda term that removes the node that displays it
 * from its parent and viewcontroller.
 * 
...
public class ViewRemovalVisitor implements LambdaTermVisitor {
    ...
    /**
     * Visits the given lambda application, traverses to the children and then
    ...
    /**
     * Visits the given lambda abstraction, traverses to the child and then
     * removes this node from its parent.
     * 
     * @param node
    ...
    public void visit(LambdaAbstraction node) {
        ...
        assert (viewController.hasNode(node)); // Abstractions always have an
                                               // explicit display
        LambdaNodeViewController actor = viewController.getNode(node);
        actor.getParentNode().removeChild(actor);
    }
    ...
    public void visit(LambdaVariable node) {
        assert (viewController.hasNode(node)); // Variables always have an
                                               // explicit display
        LambdaNodeViewController actor = viewController.getNode(node);
        actor.getParentNode().removeChild(actor);
    }
