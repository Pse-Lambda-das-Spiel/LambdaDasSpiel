...
/**
 * Represents a visitor on a lambda term that checks whether a color is bound in
 * the term. Can only visit a valid lambda term.
 * 
...
public class IsColorBoundVisitor extends ValidLambdaTermVisitor<Boolean> {
    ...
    /**
     * Visits the given lambda abstraction and checks if the color is bound
     * here. Then traverses to the parent node.
     * 
     * @param node
     *            the abstraction to be visited
     * @throws InvalidLambdaTermException
     *             if the visited abstraction binds the color that is being
     *             checked
     */
    @Override
     ...
     * 
     * @return true if the given color is bound in the visited term, false
     *         otherwise
     */
    @Override
