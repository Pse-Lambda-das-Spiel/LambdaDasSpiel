...
/**
 * LevelManager manages everything which is needed for the level contexts. This
 * manager loads all levels, difficulty settings and several assets which are
 * used in every level as well as the mapping of variables to colors and
 * reversed.
 *
...
public class LevelManager {
    ...
    private LevelManager() {
        ...
        difficultyMusicFilePaths = LevelLoadHelper
                .loadAllDifficultyMusicFilePaths();
        difficultyBGImageFilePaths = LevelLoadHelper
                .loadAllDifficultyBGImageFilePaths();
    }
     ...
     * 
     * @param id
     *            the id of the level which will be returned
    ...
    public LevelModel getLevel(int id) throws InvalidLevelIdException {
        if (id >= 0 && id <= getNumberOfLevels()) {
            LevelModel level = assetManager.get(levelFilePaths[id],
                    LevelModel.class);
            return level;
        }
     ...
     * 
     * @param id
     *            the id of the difficulty settings which whill be returned
     * @return the difficulty settings with this id
     */
    public DifficultySetting getDifficultySetting(int id) {
        DifficultySetting difficultySetting = assetManager.get(
                difficultySettingFilePaths[id - 1], DifficultySetting.class);
    ...
    public void queueAssets(AssetManager assets) {
        ...
        assets.setLoader(DifficultySetting.class, new DifficultySettingLoader(
                new InternalFileHandleResolver()));
    ...
    /**
     * Returns the mapping of variables to colors (as hexstrings) with the
     * variables as key.
     * 
    ...
    /**
     * Returns the mapping of colors (as hexstrings) to variables with the
     * colors as key.
     * 
     ...
     * 
     * @param color
     *            the to be converted color
     * @return the specific variable for the given color
     * @throws IllegalArgumentException
     *             if there is no variable stored for the given color
     */
    public static char convertColorToVariable(Color color) {
        ...
        if (colorsToVariables.get(hexString) == null) {
            throw new IllegalArgumentException(
                    "There is no variable for the color: " + color.toString());
        }
