...
/**
 * This class is used to manage all achievements. It contains a collection of
 * all achievements in the game. It can also initialize and check all
 * achievement or add or remove a single achievement.
 * 
...
public class AchievementManager {
    ...
    /**
     * Returns a Map of all achievement numbers per type, with the achievement
     * type as key.
     * 
     ...
     * 
     * @param assets
     *            the AssetManager needed for loading the resources needed for
     *            initializing the achievements.
    ...
    public void resetAchievements(AssetManager assets) {
        ...
        Collection<AchievementModel> achievementCollection = achievements
                .values();
    ...
    /**
     * Checks all achievements with the current {@link StatisticModel}. Checks
     * whether their requirements are met or not. Sets the
     * locked-/unlocked-state of the achievements accordingly.
     * 
     */
    ...
    /**
     * Adds a new achievement to the collection of all achievements, if it is
     * not already there.
     * 
     * @param achievement
     *            the to be added achievement
     * @throws IllegalArgumentException
     ...
     * 
     * @param id
     *            the id of the to be removed achievement
     */
    ...
    /**
     * Loads all achievements and the achievement types and the number of
     * achievements per type. Loads also the achievements into their specific
     * {@link AchievementViewController}.
     * 
     * @param achievementVCList
     *            the list for the {@link AchievementViewController}
     * @throws IllegalArgumentException
     *             if achievementVCList is null
     */
    public void loadAchievements(
            List<AchievementViewController> achievementVCList) {
        if (achievementVCList == null) {
            throw new IllegalArgumentException(
                    "The achievementVCList cannot be null!");
        }
        achievementTypeList = new ArrayList<>();
        achievementNumberPerType = new HashMap<>();
        FileHandle file = Gdx.files
                .internal("data/achievements/achievementList.json");
        ...
        for (JsonValue entry = value.get("achievementTypes").child(); entry != null; entry = entry.next) {
            ...
            default:
                throw new InvalidJsonException(
                        "The achievement type is invalid!");
            }
    ...
    private void loadAchievementList(JsonValue value,
            List<AchievementViewController> achievementVCList) {
        for (JsonValue entry = value.child(); entry != null; entry = entry.next) {
            ...
            case "gea":
                achievement = new GemsEnchantedAchievementModel(
                        entry.getInt("req"));
                str = "gea";
                break;
            case "lea":
                achievement = new LambsEnchantedAchievementModel(
                        entry.getInt("req"));
                str = "lea";
                break;
            case "gpa":
                achievement = new GemsPlacedAchievementModel(
                        entry.getInt("req"));
                str = "gpa";
                break;
            case "lpa":
                achievement = new LambsPlacedAchievementModel(
                        entry.getInt("req"));
                str = "lpa";
                break;
            case "gepla":
                achievement = new GemsEnchantedPerLevelAchievementModel(
                        entry.getInt("req"));
                str = "gepla";
                break;
            case "lepla":
                achievement = new LambsEnchantedPerLevelAchievementModel(
                        entry.getInt("req"));
                str = "lepla";
                break;
            case "gppla":
                achievement = new GemsPlacedPerLevelAchievementModel(
                        entry.getInt("req"));
                str = "gppla";
                break;
            case "lppla":
                achievement = new LambsPlacedPerLevelAchievementModel(
                        entry.getInt("req"));
                str = "lppla";
                break;
            case "la":
            ...
            case "ha":
                ...
                break;
            case "da":
                achievement = new DifficultyAchievementModel(
                        entry.getInt("req"));
                str = "da";
                break;
            case "ta":
            ...
            default:
                throw new InvalidJsonException(
                        "The achievement type in the achievement list is invalid!");
            }
            achievement.setIndex(entry.getInt("index"));
            achievement.setId(str
                    + String.format("%02d", achievement.getIndex()));
