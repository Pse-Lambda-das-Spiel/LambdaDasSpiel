...
/**
 * This class saves the delta statistic data gained while playing a level. It is also responsible for processing this
 * data and saving it in the StatisticModel. It only saves the statistic data if the level is finished.
 * 
 * @author Robert Hochweiss
 */
public class DeltaStatisticProcessor implements EditorModelObserver, ReductionModelObserver, LambdaTermObserver {
	...
	public void replaceTerm(LambdaTerm oldTerm, LambdaTerm newTerm) {
		if ((oldTerm == null) && (newTerm == null)) {
			throw new IllegalArgumentException("oldterm and newTerm cannot be both null!");
		}
		if (inEditorMode) {
			...
			if (newTerm != null) {
				...
				if (oldTerm != null) {
					...
					Map<String, Integer> resultMapOld = oldTerm.accept(visitor2);
					/*
					 * if the old term is not null the difference in the number of lambs and gems between the new and
					 * old lambda terms has to be calculated
					 */
					int lambsResult = resultMapNew.get("lambs") - resultMapOld.get("lambs");
					lambsPlaced += lambsResult;
					int gemsResult = resultMapNew.get("gems") - resultMapOld.get("gems");
	...
	public void variableReplaced(LambdaVariable variable, LambdaTerm replacing) {
		if ((variable == null) || (replacing == null)) {
			throw new IllegalArgumentException("neither variable nor replacing can be null!");
		}
		if (inEditorMode) {
			inEditorMode = false;
		}
	...
	public void reductionFinished(boolean levelComplete) {
		...
		if (levelComplete) {
			statistic.setSuccessfulLevelTries(statistic.getSuccessfulLevelTries() + 1);
			if (!hintUsed) {
				statistic.setHintsNotUsed(statistic.getHintsNotUsed() + 1);
			}
		}
		// only update these values if they are bigger, so that the statistic observers do not get notified needlessly
		if (gemsEnchanted > 0) {
